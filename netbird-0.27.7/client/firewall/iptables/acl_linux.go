...
func (m *aclManager) seedInitialEntries() {
	...
	*/
	// -A INPUT ! -s 100.118.0.0/16 -d 100.118.0.0/16 -i wt0 -j ACCEPT
	/*
		m.appendToEntries("INPUT",
			[]string{"-i", m.wgIface.Name(), "-s", m.wgIface.Address().String(), "!", "-d", m.wgIface.Address().String(), "-j", "ACCEPT"})
	*/
	...
	// -A INPUT -s 100.118.0.0/16 -d 100.118.0.0/16 -i wt0 -j NETBIRD-ACL-INPUT
	...
	// -A OUTPUT ! -s 100.118.0.0/16 -d 100.118.0.0/16 -o wt0 -j ACCEPT
	...
	// -A OUTPUT -s 100.118.0.0/16 -d 100.118.0.0/16 -o wt0 -j NETBIRD-ACL-OUTPUT
	...
	// -A PREROUTING ! -s 100.118.0.0/16 -d 100.118.139.87/32 -i wt0 -m mark --mark 0x7e4

	m.appendToEntries("NETBIRD-RT-NAT", []string{"-t", "nat", "-s", m.wgIface.Address().String(), "-j", "MASQUERADE"})
	m.appendToEntries("NETBIRD-RT-NAT", []string{"-t", "nat", "-d", m.wgIface.Address().String(), "-j", "MASQUERADE"})
